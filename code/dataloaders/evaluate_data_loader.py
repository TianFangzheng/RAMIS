...
def get_paths_and_transform(test_path):
    transform = val_transform
    glob_rgb = os.path.join(test_path)
    def get_gt_paths(p):
        ps = p.split('/')
...
class Evaluate():
    """A data loader for the evaluate dataset"""
    def __init__(self, test_path):
        paths, transform = get_paths_and_transform(test_path)
        self.paths = paths
        self.transform = transform

    def __getraw__(self, index):
        rgb = rgb_read(self.paths['rgb'][index])
        target = gt_read(self.paths['gt'][index])
        return rgb, target
